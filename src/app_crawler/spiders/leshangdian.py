...
class LeshangdianSpider(CrawlSpider):
    ...
    """
    client_id = (
        "21552078-1a2-2-9999-1-3-1_240_i100000000000012t19700101000000001_c25434d1p4"
    )
    ...
    def parse_start_url(self, response, **kwargs):
        ...
        c = json.loads(
            s.replace(
                ";__NEXT_LOADED_PAGES__=[];__NEXT_REGISTER_PAGE=function(r,"
                "f){__NEXT_LOADED_PAGES__.push([r, f])}",
                "",
        ...
        for link in links:
            yield scrapy.Request(
                url=link.url,
                callback=self.extract_app_link,
                meta={
                    "category": link.text,
    ...
    def extract_app_link(self, response):
        page_max = min(
            int(response.css(".page-num > .num:not(.active)::text").extract_first()),
            self.MAX_PAGE,
        )
        self.logger.info(
            f"Page [{response.meta.get('frompage')}/{page_max}]"
            f"(MAX:{page_max})"
        ...
        if (
            ...
            and next_url
            and "下一页" in response.css(".page-wrapper>a::text").extract()
        ):
    ...
    def parse_app_meta(self, response):
        ...
        try:
            ...
            if self.COUNT_APP_NUM:
                ...
                self.app_num += 1
                self.logger.info(
                    f"{self.app_num} apps total, {self.week_updates} week updates total"
                )
    ...
    def parse_comments(self, response):
        ...
        try:
            ...
            if not response.meta.get("client_id"):
                self.logger.warning("Cannot get comment without client id!")
                raise DontCloseSpider()
            dt = c["data"]["datalist"]
            ...
            if self.CRAWL_COMMENT and dt:
                ...
                if current_num <= total_num and current_num <= self.MAX_COMMENT_OFFSET:
                    yield scrapy.Request(
                        ...
                        f"&clientid={response.meta.get('client_id')}",
                        callback=self.parse_comments,
                        meta={
                            "app_id": response.meta.get("app_id"),
                            "offset": current_num,
            ...
            self.logger.warning(
                f"Error in parsing comments when trying to access url: {response.url}"
            )
