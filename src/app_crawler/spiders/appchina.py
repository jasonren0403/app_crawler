...
class AppchinaSpider(CrawlSpider):
    ...
    start_urls = [
        f"http://www.appchina.com/category/40/1_1_{PAGE_OFFSET or 1}_1_0_0_0.html",
    ...
    def parse_start_url(self, response, **kwargs):
        ...
        # print(response.meta)
        source = response.meta.get(
            "source", "soft" if response.url.split("/")[-2] == "30" else "game"
        )
        ...
        updates = len(
            [
                parse(i)
                for i in date_list
                if (datetime.datetime.now() - parse(i)).days <= 7
            ]
        )
        if next_link and self.COUNT_APP_NUM:
            ...
            self.logger.info(
                f"{source}-{app_count} apps total, {self.week_updates} week updates (+{updates})"
            )
    ...
    def parse_item_app(self, response):
        if not self.COUNT_APP_NUM:
            self.logger.info(
                f"Parsing app[{response.url.split('/')[-1]}] meta contents"
            )
        import re
        ...
        if self.CRAWL_META:
            ...
            app_meta = response.css(
                ".app-other-info-intro> .art-content::text"
            ).extract()
            ...
            try:
                app_meta = {
                    ...
                    for val in app_meta
                    if re.match(r"([\u4e00-\u9fa5\w]+)：(.+)", val)
                }
                self.logger.info(f"App-meta:{app_meta}")
                itemLoader.add_value(
                    "update_time",
                    app_meta.get("更新", "") or app_meta.get("update", ""),
                )
                itemLoader.add_value(
                    "category", app_meta.get("分类", "") or app_meta.get("category", "")
                )
                if "开发者" in app_meta or "developer" in app_meta:
                    developer = app_meta.get("开发者", "") or app_meta.get(
                        "developer", ""
                    )
                    itemLoader.add_value("developer", developer)
                else:
                    text = response.css(
                        ".app-other-info-intro>div>p::text"
                    ).extract_first()
                    if text and text.strip() == "开发者：":
                        developer = response.css(
                            ".app-other-info-intro>div>a::text"
                        ).extract_first()
                        itemLoader.add_value("developer", developer)
                        # download times not available in appchina
                        itemLoader.add_value("download_times", -1)

                        item_script = response.xpath(
                            "/html/body/script"
                        ).extract_first()
                        # var md5 = xxxxxx
                        # var packagename = 'xxxxx'
                        package_name = re.findall(
                            r"var\s+packagename\s+=\s+'([\w\.]+)'", item_script, re.M
                        )
                        itemLoader.add_value(
                            "app_id", package_name[0] or response.url.split("/")[-1]
                        )
                        # if needed
                        app_md5 = re.findall(
                            r"var\s+md5\s+=\s+'(\w+)'", item_script, re.M
                        )
                        if not app_md5:
        ...
        if self.CRAWL_COMMENT and remark_link:
            yield scrapy.Request(
                url=response.urljoin(remark_link),
    ...
    def parse_comments(self, response):
        ...
        self.logger.info(
            f"Parsing app[{response.url.split('/')[-2]}] comments. Page [{current_page}]"
        )
        for comment in response.css(".comments-list>li").extract():
            ...
            itemLoader.add_value(
                "app_id", response.meta.get("appid", response.url.split("/")[-2])
            )
