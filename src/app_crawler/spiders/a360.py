...
class A360Spider(CrawlSpider):
    ...
    def max_page(self, response):
        ...
        try:
            ...
            context.execute(
                f"{'function showPages(){};showPages.prototype.printHtml=function(){};'}{has_pg_js}"
            )
            ending_page = min(int(context.pg.pageCount), self.CRAWL_TO)
        except Exception:
            ending_page = self.MAX_PAGE_DEFAULT
            self.logger.warning(
                f"cannot get max page for {response.meta['source']}, starting from page 1"
            )
        finally:
            starting_page = self.PAGE_OFFSET if self.PAGE_OFFSET <= ending_page else 1
            self.logger.info(
                f"{response.meta['source']}: from page {starting_page} to {ending_page}"
            )
            for i in range(starting_page, ending_page + 1):
                ...
                yield scrapy.Request(
                    ...
                    callback=self.from_pages,
                    dont_filter=True,
                )

    def from_pages(self, response):
        link_ext = LinkExtractor(
            restrict_css=".icon_box>.iconList>li>h3", allow=r"/detail/index/soft_id/\d+"
        )
    ...
    def parse_item_app(self, response):
        ...
        for script in scripts:
            if "详情页的命名空间" in script:
                detail = re.findall(
                    r"var\s+detail\s+=\s+\(function\s+\(\)\s+{\s+return\s+({[\w\W]*});\s+}\)",
                    script,
                    re.M,
                )
                break
        ...
        try:
            ...
            if self.COUNT_APP_NUM:
                base_info = re.sub(
                    r"[\t\r\n\s]+",
                    " ",
                    remove_tags(response.css(".base-info").extract_first()),
                )
                match_public_time = re.findall(
                    r"更新时间：(\d{4}-(0\d|1[012])-([012]\d|3[01]))", base_info
                )
                import datetime
                ...
                if (
                    match_public_time
                    and (datetime.datetime.now() - parse(match_public_time[0][0])).days
                    <= 7
                ):
                    self.week_updates += 1
                self.app_count += 1
                self.logger.info(
                    f"{self.app_count} apps total, {self.week_updates} week updates"
                )
            if self.CRAWL_META:
                ...
                score = response.css(
                    "#app-info-panel span.s-1.js-votepanel::text"
                ).extract_first()
                ...
                if not response.css(".breif::text"):
                    ...
                    itemLoader.replace_css(
                        "description",
                        '#html-brief>p:not([style="text-align:center"])',
                        Join(""),
                    )
                    if not response.css(
                        '#html-brief>p:not([style="text-align:center"])'
                ...
                if not (match_developer and match_public_time):
                    raise DropItem(
                        f"No developer or public_time, don't scrap this item {response.url}"
                    )
            ...
            if self.CRAWL_COMMENT:
                yield scrapy.Request(
                    ...
                    callback=self.parse_comment_app,
                    dont_filter=True,
                    meta={
                        "sid": sid,
                        "type": type,
            ...
            raise DropItem(
                f"Corrupted app item! Bad app items total: {self.bad_app_count}"
            )
            ...
            if comment_list:
                ...
                for comment in comment_list:
                    ...
                    try:
                        ...
                        itemLoader = AppCommentLoader(
                            item=AppComment(), selector=selector
                        )
