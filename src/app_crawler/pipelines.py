        ...
        try:
            ...
            if "user" in item:
                ...
                if repetition:
                    ...
                    if dl_times == -1:
                        spider.logger.warning(
                            f"update bad dl_time value to {item['download_times']}"
                        )
                    if rev_times == -1:
                        spider.logger.warning(
                            f"update bad rev_time value to {item['review_times']}"
                        )
                    values = (
                        item["name"],
                        item["store"],
                    ...
                    sql = (
                        "REPLACE INTO app_meta VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
                    )
                else:
                    values = (
                        item["name"],
                    ...
                    sql = (
                        "INSERT INTO app_meta VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
                    )

            db_cur.execute(sql, values)
            spider.logger.info(
                f"source:{item['store']} appid:{item['app_id']} ->DB table:{goal}"
            )
            self.db_conn.commit()
            db_cur.close()
